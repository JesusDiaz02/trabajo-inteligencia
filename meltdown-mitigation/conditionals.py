...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    if Efficiency >= 80:
        return 'green'
    elif Efficiency >=60  <=80:
        return 'orange'
    elif Efficiency >=30 <=60 :
        return 'red'
    elif Efficiency <=30:
        return 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    values = temperature * neutrons_produced_per_second

    if values < threshold * 0.90:
        return 'LOW'
    elif 0.9 * threshold <= values <= 1.10 * threshold:
        return 'NORMAL'
    else:
        return 'DANGER'
    return str()